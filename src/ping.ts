...
export async function ping(queue: PQueue, url: string, secretKey: string) {
    ...
    try {
        ...
        await postWebhook(queue, url, body, signature, timestamp, {maximumAttempts: 0});
    } catch (e) {
    ...
    try {
        ...
        await postWebhook(queue, url, body, invalidSignature, timestamp, {maximumAttempts: 0});
