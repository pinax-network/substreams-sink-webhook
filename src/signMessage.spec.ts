...
test("signMessage", () => {
    ...
    assert.equal(sig, "a2e1437d2b32774418f46365d4dccb4509be5469ed24ba0d1707ce4ca76dd7fbe0b01597d9c91391fba5316e917d4dca3134a6c1f2c283d708c02cd33d5b080d")
...
test("signMessage - special characters", () => {
    ...
    const timestamp = 1686865337
    const sig = signMessage(timestamp, body, secretKey);
    assert.equal(sig, "58033ab867ff3be7eaba373a50ea8a21b716ef5b0cbab8663e48e82ad6694eec17281c132ccde4dbe61ff19e2263513e265a2da90de8748e7c70c818d489cc04")
    const msg = Buffer.from(timestamp + body);
    assert.equal(verify(msg, sig, publicKey), true);
});