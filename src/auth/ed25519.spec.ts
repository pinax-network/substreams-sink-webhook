...
describe("sign", () => {
  test("sign", () => {
    const signature = auth.sign(timestamp, body, secretKey);
    expect(signature).toBe(signature);
  });

  test("error - missing body", () => {
    expect(() => auth.sign(timestamp, "", secretKey)).toThrow("missing body");
  });

  test("error - invalid secret key length", () => {
    expect(() => auth.sign(timestamp, body, "")).toThrow("invalid secret key length");
  });

  test("error - invalid timestamp", () => {
    expect(() => auth.sign(0, body, secretKey)).toThrow("invalid timestamp");
    expect(() => auth.sign(Number("abc"), body, secretKey)).toThrow("invalid timestamp");
  });
});
